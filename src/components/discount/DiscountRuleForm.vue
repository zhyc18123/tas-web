            ...
            <div class="am-form-group">
              ...
              <div class="am-u-sm-3 input-field  am-u-end">
                <select2 required v-model="formData.categoryId">
                  ...
                  <option v-for="item in discountRuleList" :value="item.discountCategoryId">{{item.name}}</option>
                </select2>
              </div>
            </div>
  ...
  export default{
    ...
    methods: {
      save: function (complete) {
        ...
        var _this = this
        _this.formData.effectiveAreaBus = JSON.stringify(areaTeamOrBusTeams)
      ...
      loadDiscountCategoryList: function () {
        io.post(io.apiAdminDiscountCategoryList, {
          ...
          (ret) => {
            if (ret.success) {
              this.discountRuleList = ret.data.list
            } else {
              this.$alert(ret.desc)
            }
          },
