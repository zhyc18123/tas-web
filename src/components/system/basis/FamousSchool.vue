  ...
  <div class="topic-origin">
    <div class="content">
      ...
      <el-table :data="tableData" style="width: 100%">
        <el-table-column label="名校设定" align="center">
          <template scope="scope">
            ...
            <div v-if="scope.row.addRow" class="btn-add">
              <el-input v-model="newName" placeholder="请输入名校名称"></el-input>
            </div>
        ...
        <el-table-column align="center" label="操作">
          <template scope="scope">
            ...
            <el-button v-show="scope.row.isEdit" class="btn-delete" @click="scope.row.isEdit = false;questionSchoolList()">取消</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    questionSchoolList() {
      io.post(io.questionSchoolList, { subjectName: '' }, (data) => {
    ...
    handleSave(row) {
      ...
      if (row) {
        ...
        if (row.name.length>20) {
          this.$msgbox.alert('名校名称不能超过20个字')
          return
        }
      ...
      io.post(io.saveQuestionOriginSchool, data, (data) => {
        this.$message('保存成功！')
        this.handleCancel()
        this.questionSchoolList()
        this.newName = '';
