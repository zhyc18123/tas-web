      ...
      <div class="widget-body  am-fr">
          ...
          <div class="am-form-group">
            <div class="am-btn-toolbar">
              <div class="am-btn-group am-btn-group-xs">
                <button type="button" class="am-btn am-btn-default am-btn-success" @click="$router.push('/main/sys/room/add?campusId=' + query.campusId)" v-if="hasPermission('add')"><span class="am-icon-plus"></span>新增</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="am-u-sm-12 am-scrollable-horizontal">
          <table width="100%" class="am-table am-table-bordered am-table-compact am-table-striped am-text-nowrap">
            ...
            <tr v-for="item in tableData" :key="item.roomId">
              <td>{{item.roomName}}</td>
              <td>{{item.seatAmount }}</td>
              <td>{{item.rent}}</td>
              <td>{{item.status == '0' ? '未启用' :'正常'}}</td>
              <td>
                <div class="tpl-table-black-operation">
                  <a href="javascript:;" @click="$router.push('/main/sys/room/edit/'+item.roomId)" v-if="hasPermission('edit')">
                    <i class="am-icon-edit"></i> 编辑
                  </a>

                  <a  href="javascript:;" class="tpl-table-black-operation-danger" @click="del(item.roomId)" v-if="hasPermission('del')">
